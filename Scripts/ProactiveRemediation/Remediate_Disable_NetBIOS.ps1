<#
.DESCRIPTION
	Remediation script will try to disable NetBIOS on the client.
...
# Proactive Remediation Script
	...
	try {
		$DisableNetBIOS = Set-ItemProperty HKLM:\SYSTEM\CurrentControlSet\services\NetBT\Parameters\Interfaces\tcpip* -Name NetbiosOptions -Value 2 -ErrorAction SilentlyContinue
		...
		if (!($Counter -eq 0)) {
			$Msg = "NetBIOS was successfully disabled."
			Write-Host $Msg
			Write-Log -Message "[$($Subject)]: $($Msg)"
			exit 0
		}
		else {
			$Msg = "The Proactive Remediation script failed to disable NetBIOS."
			Write-Host $Msg
