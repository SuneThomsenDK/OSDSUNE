<#
.DESCRIPTION
	Detection script will check if the correct regional settings has been applied to the client.
...
# Proactive Remediation Script
	...
	$Subject = "Set Regional Settings (da-DK)"

	try {
		$CheckCulture = Get-Culture
		$CheckTimeZone = Get-TimeZone
		$Counter = 0

		if (!($CheckCulture.Name -eq "da-DK")) {$Counter+=1}
		if (!($CheckTimeZone.Id -eq "Romance Standard Time")) {$Counter+=1}

		if (($Counter -eq 0)) {
			$Msg = "Regional settings are correct, do nothing."
			Write-Host $Msg
			Write-Log -Message "[$($Subject)]: $($Msg)"
			exit 0
		}
		else {
			$Msg = "Regional settings are not correct. Starting remediation script..."
			Write-Host $Msg
