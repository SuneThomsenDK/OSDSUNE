<#
.DESCRIPTION
	Detection script will check if [This PC] shortcut is present on the clients desktop.
...
# Proactive Remediation Script
	...
	try {
		...
		if (($RegistryValue -eq "0")) {
			$Msg = "This PC shortcut was found, do nothing."
			Write-Host $Msg
			Write-Log -Message "[$($Subject)]: $($Msg)"
			exit 0
		}
		else {
			$Msg = "This PC shortcut was not found. Starting remediation script..."
			Write-Host $Msg
