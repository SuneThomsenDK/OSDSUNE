<#
.DESCRIPTION
	Detection script will check if the screen capture protection is enabled on the Cloud PC.
...
# Proactive Remediation Script
	...
	try {
		...
		if (($CurrentRegistryValue -eq $RegistryValue)) {
			...
			$Msg = "The screen capture protection was not enabled. Starting remediation script..."
			Write-Host $Msg
