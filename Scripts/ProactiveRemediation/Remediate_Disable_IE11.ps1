<#
.DESCRIPTION
	Remediation script will try to disable Internet Explorer 11 on the client.
...
# Proactive Remediation Script
	...
	$Subject = "Disable Internet Explorer 11"

	try {
        $DisableIE11 = Disable-WindowsOptionalFeature -FeatureName Internet-Explorer-Optional-amd64 -Online -NoRestart | Out-Null
		$Check = Get-WindowsOptionalFeature -Online | Where-Object {$_.FeatureName -eq "Internet-Explorer-Optional-amd64"} -ErrorAction SilentlyContinue

        If (($Check.State -eq "DisablePending")) {
		    $Msg = "Internet Explorer 11 was successfully disabled."
		    Write-Host $Msg
		    Write-Log -Message "[$($Subject)]: $($Msg)"
		    exit 0
        }
		else {
			$Msg = "The Proactive Remediation script failed to disable Internet Explorer 11."
			Write-Host $Msg
