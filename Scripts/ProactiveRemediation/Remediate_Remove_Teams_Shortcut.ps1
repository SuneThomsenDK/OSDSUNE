<#
.DESCRIPTION
	Remediation script will try to delete Microsoft Teams shortcut(s) from the desktop on the client.
...
# Proactive Remediation Script
	...
	$Subject = "Delete Teams Shortcut"

	try {
		$ShortcutName = "Microsoft Teams"
		$Desktop = [Environment]::GetFolderPath("Desktop")
		$DeleteShortcut = Get-ChildItem $Desktop -Recurse | Where-Object Name -like "*$ShortcutName*.lnk" | Remove-Item
		$Search = Get-ChildItem $Desktop -Recurse | Where-Object Name -like "*$ShortcutName*.lnk"

		if (($Search -eq $Null)) {
			$Msg = "Microsoft Teams shortcut(s) was successfully deleted."
			Write-Host $Msg
			Write-Log -Message "[$($Subject)]: $($Msg)"
			exit 0
		}
		else {
			$Msg = "The Proactive Remediation script failed to delete Microsoft Teams shortcut(s)."
			Write-Host $Msg
