<#
.DESCRIPTION
	Remediation script will try to enable [This PC] shortcut on the clients desktop.
...
# Proactive Remediation Script
	...
	$Subject = "Enable This PC Shortcut"
	...
	try {
		...
		$SetRegistryValue = Set-ItemProperty $RegistryPath -Name $RegistryName -Value 0 -ErrorAction SilentlyContinue

		$GetRegistryValue = Get-ItemProperty $RegistryPath -Name $RegistryName -ErrorAction SilentlyContinue
		$RegistryValue = $GetRegistryValue."{20D04FE0-3AEA-1069-A2D8-08002B30309D}"

		if (($RegistryValue -eq "0")) {
			$Msg = "This PC shortcut was successfully enabled."
			Write-Host $Msg
			Write-Log -Message "[$($Subject)]: $($Msg)"
			exit 0
		}
		else {
			$Msg = "The Proactive Remediation script failed to enable This PC shortcut."
			Write-Host $Msg
