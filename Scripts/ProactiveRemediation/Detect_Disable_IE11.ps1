<#
.DESCRIPTION
	Detection script will check if Internet Explorer 11 has been disabled on the client.
...
# Proactive Remediation Script
	...
	try {
        ...
        If (!($Check.State -eq "Enabled")) {
		    $Msg = "Internet Explorer 11 is disabled, do nothing."
		    Write-Host $Msg
		    Write-Log -Message "[$($Subject)]: $($Msg)"
		    exit 0
        }
		else {
			$Msg = "Internet Explorer 11 is enabled. Starting remediation script..."
			Write-Host $Msg
