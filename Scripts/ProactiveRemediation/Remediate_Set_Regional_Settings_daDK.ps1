<#
.DESCRIPTION
	Remediation script will try to apply the correct regional settings to the client.
...
# Proactive Remediation Script
	...
	try {
		...
		if (!($CheckCulture.Name -eq "da-DK")) {
			Set-Culture da-DK
			$Counter+=1
		}

		if (!($CheckTimeZone.Id -eq "Romance Standard Time")) {
			Set-TimeZone -Id "Romance Standard Time"
			$Counter+=1
		}

		if (!($Counter -eq 0)) {
			$Msg = "Regional settings was successfully applied."
			Write-Host $Msg
			Write-Log -Message "[$($Subject)]: $($Msg)"
			exit 0
		}
		else {
			$Msg = "The Proactive Remediation script failed to apply the regional settings."
			Write-Host $Msg
