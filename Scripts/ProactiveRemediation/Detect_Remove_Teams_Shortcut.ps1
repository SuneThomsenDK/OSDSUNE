<#
.DESCRIPTION
	Detection script will check if Microsoft Teams shortcut(s) is present on the clients desktop.
...
# Proactive Remediation Script
	...
	try {
		...
		if (($Search -eq $Null)) {
			$Msg = "Microsoft Teams shortcut(s) not found, do nothing."
			Write-Host $Msg
			Write-Log -Message "[$($Subject)]: $($Msg)"
			exit 0
		}
		else {
			$Msg = "Microsoft Teams shortcut(s) found. Starting remediation script..."
			Write-Host $Msg
